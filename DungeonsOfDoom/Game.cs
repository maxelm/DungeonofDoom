...
using DoDLibrary.GameObjects.Characters.Players;
using DoDLibrary.GameObjects.Rooms;
using DoDLibrary.GameObjects.Items.Weapons;
using DoDLibrary.GameObjects.Items;
using DoDLibrary.GameObjects.Items.Consumables;
...
namespace DungeonsOfDoom
{
    class Game
    {
        ...
        private void Combat(Monster opponent) //todo Add Combat Logic
        {
            ...
            int roundCounter = 1;
            Console.WriteLine($"\nA wild {TextUtils.DisplayName(opponent)} appeared! Prepare yourself!"); //todo add name of class to message
            ...
            if (opponent.IsWillingToFight(player))
            {
                ...
                if (player.Health < 0)
                {
                    Console.WriteLine($"You've been slain by {TextUtils.DisplayName(opponent)}");
                }
                else
                {
                    Console.WriteLine($"You slayed {TextUtils.DisplayName(opponent)}");
                }
            }
            else
            {
        ...
        private void DisplayBackpack()
        {
            ...
            {
                Console.WriteLine($"{TextUtils.DisplayName(item)}");
